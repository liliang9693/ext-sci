...
class DFRobot_SUAB_IIC(DFRobot_SUAB):
  ...
  def _send_packet(self, pkt):
    ...
    #print("sendpacket _addr=",self._addr)
    #print("sendpacket pkt=",pkt)
    '''
    for data in pkt:
      try:
        #self._bus.write_byte(self._addr, data)
        self._bus.writeto(_addr, data)
    ...
    try:
      #self._bus.write_byte(self._addr, data)
      self._bus.writeto(self._addr, pkt)
    except:
      print("send packet except:",sys.exc_info()[0])
  ...
  def _recv_data(self, len):
    ...
    while i < len:
      try:
        ...
        rslt[i] = self._bus.readfrom(self._addr,1)
    ...
    try:
      ...
      rslt = self._bus.readfrom(self._addr, len)
    except:
      print("recv data  except:",sys.exc_info()[0])
